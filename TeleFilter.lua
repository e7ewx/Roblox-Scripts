...
local function respawn()
	...
	frame.Text = "TeleFilter Enabled"
	frame.Font = "FredokaOne"
	frame.Name = "TeleFilter"
	frame.TextTransparency = 0.5

	part = Instance.new("Highlight")
...
event.Parent = player
event.Name = "TeleFilter"

runService.Heartbeat:Connect(function(DeltaTime)
	local fps = 1 / DeltaTime
	if character then
		part.Parent = character
		new = character.HumanoidRootPart.CFrame
		local posChange = (new.Position - last.Position).Magnitude
		local velocity = (character.HumanoidRootPart.Velocity / fps).Magnitude * 5
		if posChange > velocity and enabled and posChange > 0.5 and not legalTeleport then
			character.HumanoidRootPart.CFrame = last
			print("TeleFilter -- Blocked teleport")
			part.FillColor = Color3.new(1, 0, 0)
			part.FillTransparency = 0
		else
			part.FillTransparency += (1 - part.FillTransparency)/15
			if legalTeleport and enabled then
				part.FillTransparency = 0
				part.FillColor = Color3.new(0, 1, 0)
				print("TeleFilter -- Passed teleport")
				legalTeleport = false
			end
		end
		last = character.HumanoidRootPart.CFrame
	end

	if enabled then
		frame.Text = "TeleFilter Enabled"
	else
		frame.Text = "TeleFilter Disabled"
	end
end)
